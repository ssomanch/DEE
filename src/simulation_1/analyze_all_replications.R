	...
	if (filtered){
		seeds_configs_and_paths[,method:=plyr::mapvalues(fname,
														from=c('bias_correction_description.csv',
															...
															'cattaneo_benchmark_description.csv',
															'kallus_benchmark_description.csv'),
														to=c('Bias','CATE','Causal forest','Cattaneo','Kallus'))]
	} else {
		seeds_configs_and_paths[,method:=plyr::mapvalues(fname,
...
p = ggplot(filtered_PLP_search %>% 
			...
			mutate(relative_MSE_smooth = ifelse(`CATE lengthscale` == `Bias lengthscale`, relative_MSE, NA)),
...
Cattaneo_means = original_filtered$Cattaneo_means
...
both_sum_stats = rbind(CF_means %>% mutate(Benchmark='CF'),
					   Cattaneo_means %>% mutate(Benchmark='Cattaneo'),
